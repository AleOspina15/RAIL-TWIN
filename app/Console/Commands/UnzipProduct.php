...
class UnzipProduct extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'unzip:product {project_id} {path}';
    ...
    public function handle()
    {
        ...
        $str_sql .= " SET mensaje = 'Preparando producto <strong>$nombreArchivo<s/trong>.', estado='Activo', notificado=FALSE, fechahora='NOW()'";
        $str_sql .= " WHERE id_proyecto = $project_id AND nombre = '$nombreArchivo'";
        $res = DB::select($str_sql);
        ...
        $str_sql = "UPDATE sch_aicedronesdi.job";
        $str_sql .= " SET mensaje = 'Procesando producto <strong>$nombreArchivo</strong>.', estado='Activo', notificado=FALSE, fechahora='NOW()'";
        $str_sql .= " WHERE id_proyecto = $project_id AND nombre = '$nombreArchivo'";
        $res = DB::select($str_sql);
        ...
        foreach ($results as $clave=>$valor) {
            ...
            $str_sql .= " VALUES($project_id,'$filepath','Procesando archivo <strong>$filepath</strong>','Activo')";
            $res = DB::select($str_sql);
            ...
            if ($type === "point_cloud") {
                ...
                if (count($res_existe) > 0) {
                    $str_sql = "UPDATE sch_aicedronesdi.job";
                    $str_sql .= " SET mensaje = 'Ya existe una capa con el nombre <strong>$name_o</strong>', estado='Error', notificado=FALSE, fechahora='NOW()'";
                    $str_sql .= " WHERE id_proyecto = $project_id AND nombre = '$filepath'";
                    $res = DB::select($str_sql);
                } else {
                    ...
                    $str_sql = "UPDATE sch_aicedronesdi.job";
                    $str_sql .= " SET mensaje = 'Publicando <strong>$name_o</strong>.', estado='Activo', notificado=FALSE, fechahora='NOW()'";
                    $str_sql .= " WHERE id_proyecto = $project_id AND nombre = '$filepath'";
                    $res = DB::select($str_sql);
                    ...
                    $str_sql = "UPDATE sch_aicedronesdi.job";
                    $str_sql .= " SET mensaje = 'Capa <strong>$name_o</strong> publicada correctamente.', estado='Terminado', notificado=FALSE, fechahora='NOW()'";
                    $str_sql .= " WHERE id_proyecto = $project_id AND nombre = '$filepath'";
                    $res = DB::select($str_sql);
        ...
        $str_sql = "UPDATE sch_aicedronesdi.job";
        $str_sql .= " SET mensaje = 'El proceso de publicación del producto <strong>$nombreArchivo</strong> ha teminado.', estado='Finalizado', notificado=FALSE, fechahora='NOW()'";
        $str_sql .= " WHERE id_proyecto = $project_id AND nombre = '$nombreArchivo'";
        $res = DB::select($str_sql);
