...
var AvVall = new TileLayer({
    id: 'AvVall',
    title: 'Ávila - Valladolid',
    name: 'AvVall',
    source: new TileWMS({
        url: 'http://'+ip_server+':8080/geoserver/rail-twin/ows?',
        params: {
            LAYERS: 'rail-twin:ProdSolarAvVall',
            //FORMAT: 'image/png',
            VERSION: '1.3.0',
            TILED: true
        },
...
var AvMad = new TileLayer({
    ...
    source: new TileWMS({
        url: 'http://'+ip_server+':8080/geoserver/rail-twin/ows?',
        params: {
            LAYERS: 'rail-twin:ProdSolarAvMad',
            //FORMAT: 'image/png',
            VERSION: '1.3.0',
            TILED: true
        },
...
var BurMir = new TileLayer({
    id: 'BurMir',
    title: 'Burgos - Miranda de Ebro',
    name: 'BurMir',
    source: new TileWMS({
        url: 'http://'+ip_server+':8080/geoserver/rail-twin/ows?',
        params: {
            LAYERS: 'rail-twin:ProdSolarBurMir',
            //FORMAT: 'image/png',
            VERSION: '1.3.0',
            TILED: true
        },
...
function resaltarFeature(feature) {
    let coord = feature.geometry.coordinates;
    //console.log('Coordenadas originales:', coord);

    // Extraer polígonos dentro de `MultiPolygon`
    if (feature.geometry.type === "MultiPolygon") {
       coord = feature.geometry.coordinates[0][0]; // Extrae el primer polígono correctamente
    } else {
        coord = feature.geometry.coordinates; // Si es un Polígono normal, usa las coordenadas directamente
    }

    //console.log('Coordenadas corregidas para el polígono:', coord);

    // Filtrar coordenadas inválidas
    coord = coord.filter(c => Array.isArray(c) && c.length === 2 && isFinite(c[0]) && isFinite(c[1]));

    //console.log('Coordenadas filtradas:', coord);

    // Asegurar que el polígono esté cerrado

    if (coord.length >= 3 && 
        (coord[0][0] !== coord[coord.length - 1][0] || coord[0][1] !== coord[coord.length - 1][1])) {
        coord.push(coord[0]); // Cerrar el polígono manualmente
    }
    //console.log('Coordenadas listas para el polígono:', coord);

    //console.log('Coordenadas finales:', coord);

    // Si las coordenadas están vacías, evitar continuar
    if (!coord.length) {
        console.warn('Las coordenadas siguen vacías. No se generará la capa.');
        return;
    }

    // Calcular manualmente la extensión de las coordenadas para evitar `Infinity`
    let extentManual = olExtent.boundingExtent(coord);
    //console.log('Extensión manual calculada:', extentManual);

    // Crear la geometría corregida
    //console.log('Tipo de estructura de coordenadas:', Array.isArray(coord[0]) ? 'Polígono válido' : 'Formato incorrecto');

    let geometry = new Polygon([coord]); 
    geometry.transform('EPSG:3857', 'EPSG:3857');

    //console.log('Extensión corregida:', geometry.getExtent());

    // Validar que la extensión sea correcta antes de ajustar la vista
    if (extentManual[0] !== Infinity && extentManual[2] !== -Infinity) {
        map.getView().fit(extentManual, { duration: 1000 });
    } else {
        console.warn('La extensión aún es inválida, pero al menos calculamos manualmente.');
    }
    ...
    // Crear la capa resaltada
    capaResaltada = new VectorLayer({
        ...
        style: new Style({
            ...
            fill: new Fill({
                color: 'rgba(255, 255, 255, 0.2)' // Color semitransparente para visualizar el polígono
            })
        })
    });

    capaResaltada.setZIndex(9999);
    capaResaltada.set('displayInLayerSwitcher', false);
    map.addLayer(capaResaltada);

    //capaResaltada.getSource().refresh();
    map.render();
}
...
var f_obj = {
    ...
    inicio: function() {
        ...
        map.on('singleclick', function(evt){
            ...
            let irr = AllLayers.find(layer => layer.get('title') === 'Eficiencia Energética');
            let irr_arr = irr.getLayers().getArray()
            irr = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar');
            let AvMad = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Ávila - Madrid');
            let AvVall = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Ávila - Valladolid');
            let BurMir = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Burgos - Miranda del Ebro');
            if(irr.isVisible()){
                //console.log(irr);
                url = irr.getSource().getFeatureInfoUrl(
                    evt.coordinate, viewResolution, 'EPSG:3857',
                    { 'INFO_FORMAT': 'application/json' }
                );
            } else if(AvMad.isVisible()){
                url = AvMad.getSource().getFeatureInfoUrl(
                    evt.coordinate, viewResolution, 'EPSG:3857',
                    { 'INFO_FORMAT': 'application/json' }
                );
            } else if(AvVall.isVisible()){
                url = AvVall.getSource().getFeatureInfoUrl(
                    evt.coordinate, viewResolution, 'EPSG:3857',
                    { 'INFO_FORMAT': 'application/json' }
                );

            } else if(BurMir.isVisible()){
                url = BurMir.getSource().getFeatureInfoUrl(
                    evt.coordinate, viewResolution, 'EPSG:3857',
                    { 'INFO_FORMAT': 'application/json' }
                );
            }
               ...
               fetch(url)
                ...
                .then(data => {
                    if (data.features.length > 0) {
                        let solar_em;
                        let solar_hm;
                        if(data.features[0].properties['Solar_Em']){
                            solar_em = data.features[0].properties['Solar_Em'];
                        } else {
                            solar_em = data.features[0].properties['solar_em'];
                        }

                        if(data.features[0].properties['Solar_Hm']){
                            solar_hm = data.features[0].properties['Solar_Hm'];
                        } else{
                            solar_hm = data.features[0].properties['solar_hm'];
                        }
                        console.log(solar_em); 
                        console.log(solar_hm); 
                        if(solar_em && solar_hm){
                            map.addOverlay(overlayPopup);
                            popup.innerHTML = `<strong>Irradiancia Solar</strong><br>
                                                Solar EM: ${solar_em} W/m²<br>
                                                Solar HM: ${solar_hm} kWh<br>`;
                            overlayPopup.setPosition(evt.coordinate);
                            popup.style.display = 'block';
                        }
                        resaltarFeature(data.features[0]); // Llamar función para dibujar borde
                    }
                }); 
            }
        });
...
*/
        ...
        document.getElementById("btn-zoom-Esp").addEventListener("click", function() {
            ...
            irr = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar');
            irr.setVisible(true);
            irr.set('displayInLayerSwitcher', true);
            let AvMad = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Ávila - Madrid');
            let AvVall = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Ávila - Valladolid');
            let BurMir = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Burgos - Miranda del Ebro');
            AvMad.setVisible(false);
            AvVall.setVisible(false);
            BurMir.setVisible(false);
            AvMad.set('displayInLayerSwitcher', false);
            AvVall.set('displayInLayerSwitcher', false);
            BurMir.set('displayInLayerSwitcher', false);
            map.getView().animate({
                center: olProj.fromLonLat([-3.7038, 40.4168]), 
                zoom: 7, // Nivel de zoom más cercano
                duration: 1000 // Animación en milisegundos
            });
            actualizaLayerControl();
        });
        ...
        document.getElementById("btn-zoom-AvMad").addEventListener("click", function() {
            if (capaResaltada) {
                popup.style.display="none";
                map.removeLayer(capaResaltada);
            }
            ...
            irr.set('displayInLayerSwitcher', false);
            let AvMad = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Ávila - Madrid');
            let AvVall = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Ávila - Valladolid');
            let BurMir = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Burgos - Miranda del Ebro');
            AvMad.setVisible(true);
            AvVall.setVisible(false);
            BurMir.setVisible(false);
            AvMad.set('displayInLayerSwitcher', true);
            AvVall.set('displayInLayerSwitcher', false);
            BurMir.set('displayInLayerSwitcher', false);
            map.getView().animate({
                center: olProj.fromLonLat([-4.131790358312848, 40.58806944810294]), 
                zoom: 10, // Nivel de zoom más cercano
                duration: 1000 // Animación en milisegundos
            });
            actualizaLayerControl();
        });

        document.getElementById("btn-zoom-AvVall").addEventListener("click", function() {
            if (capaResaltada) {
                popup.style.display="none";
                map.removeLayer(capaResaltada);
            }
            ...
            let BurMir = irr_arr.find(layer => layer.get('title') === 'Irradiancia Solar Burgos - Miranda del Ebro');
            AvMad.setVisible(false);
            AvVall.setVisible(true);
            BurMir.setVisible(false);
            AvMad.set('displayInLayerSwitcher', false);
            AvVall.set('displayInLayerSwitcher', true);
            BurMir.set('displayInLayerSwitcher', false);
            map.getView().animate({
                center: olProj.fromLonLat([-4.911510847009722, 41.31396134150454]), 
                zoom: 9, // Nivel de zoom más cercano
                duration: 1000 // Animación en milisegundos
            });
            actualizaLayerControl();
        });

        document.getElementById("btn-zoom-BurMir").addEventListener("click", function() {
            if (capaResaltada) {
                popup.style.display="none";
                map.removeLayer(capaResaltada);
            }
            ...
            AvMad.setVisible(false);
            AvVall.setVisible(false);
            BurMir.setVisible(true);
            AvMad.set('displayInLayerSwitcher', false);
            AvVall.set('displayInLayerSwitcher', false);
            BurMir.set('displayInLayerSwitcher', true);
            map.getView().animate({
                center: olProj.fromLonLat([-3.383176492074771, 42.4878920196737]), 
                zoom: 10, // Nivel de zoom más cercano
                duration: 1000 // Animación en milisegundos
            });
            actualizaLayerControl();
